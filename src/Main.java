...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        Reader thread6 = new Reader(1000,1176);
        ...
        while(loop) {
            ...
            if (ft1.isDone() && ft2.isDone() && ft3.isDone() && ft4.isDone() && ft5.isDone() && ft6.isDone()) {
                System.out.println("Done");
                loop = false;
                executor.shutdown();
                Instant end = Instant.now();
                Duration totalTime = Duration.between(start, end);
                int totalCount = ft1.get()+ft2.get()+ft3.get()+ft4.get()+ft5.get()+ft6.get();
